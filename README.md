...
Inside Claude or Cursor, type **`/baton`** (or `/baton list`). Codex removed custom slash commands, so `/baton` is not in its built-in `/` menu. An attached terminal intercepts `/baton` before Codex sees it; `$baton` (or `/skills` → Baton) is the Codex listing. The picker opens in the attach terminal. An IDE chat for that same project uses the hook payload’s workspace (`cwd` / `workspace_roots`), not the hook process directory. Enter hops; `q` keeps the current CLI.
